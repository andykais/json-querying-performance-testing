...
const performanceClasses = {
  ...
  'json-p3': require('./queriers/json-p3'),
...
const perf = (perfClasses, datasets) => async (testLibraries, testDatasets, testMethods) => {
  const datasetPerf = {};
    ...
    for (const libName of testLibraries) {
      ...
      for (const method of testMethods) {
        try {
          ...
          console.log(`  - ${method} took ${datasetPerf[datasetName][libName][method]} seconds.`);
          if (result.length !== methodResults[method]) {
            e = new Error(`result should have length ${methodResults[method]} but instead is ${result.length}`);
            e.name = 'AssertError';
            throw e;
          }
        } catch (e) {
...
perf(performanceClasses, datasets)(testLibraries, testDatasets, testMethods).then(logPerf);
